...
namespace TFG
{
    ...
    public class FreeClimbAnimHook : MonoBehaviour
    {
        ...
        public float w_rh;//cada braç i cama
        public float w_lh;
        public float w_lf;
        public float w_rf;

        Vector3 rh, lh, rf, lf;
        public static Vector3 rh1, lh1, rf1, lf1;
        Transform h;
        bool isMirror;
        bool isLeft;
        Vector3 prevMovDir;
        ...
        public void Update()
        {
            if (ThirdPersonMovement.resetPos)
            {
                OnAnimatorIK();
            }
        ...
        void UpdateGoals(Vector3 moveDir)
        {
           ...
           /* goals.lh = moveDir.x < 0;
            goals.rf = moveDir.x > 0;
            goals.lf = moveDir.x < 0;
            goals.rf = moveDir.x > 0;*/
        }
        ...
        public IKSnapshot CreateSnapshot(Vector3 o)
        {
            ...
            //Debug.Log("Base lh: " + LocalToWorld(ikBase.lh) + " busca lh: "+ r.lh);
            return r;
        }
        ...
        {
            IKSnapshot r = new IKSnapshot();
            Vector3 _lh = ikBase.lh;
            r.lh = GetPosActual(_lh, AvatarIKGoal.LeftHand);

            Vector3 _rh = ikBase.rh;
            r.rh = GetPosActual(_rh, AvatarIKGoal.RightHand);

            Vector3 _lf = ikBase.lf;
            r.lf = GetPosActual(_lf, AvatarIKGoal.LeftFoot);

            Vector3 _rf = ikBase.rf;
            r.rf = GetPosActual(_rf, AvatarIKGoal.RightFoot);
        }*/
        ...
        Vector3 GetPosActual(Vector3 o, AvatarIKGoal goal)
        {
            ...
            //Debug.Log("Goal: " + AvatarIKGoal.LeftFoot);
            Vector3 r = o;
            Vector3 origin = o;
        ...
        {
            delta = Time.deltaTime;
            if (ThirdPersonMovement.resetPos == false)
            {
            ...
            else if (ThirdPersonMovement.resetPos)
            {
                SetIKPos(AvatarIKGoal.LeftHand, lh, 0);
                SetIKPos(AvatarIKGoal.RightHand, rh, 0);
                SetIKPos(AvatarIKGoal.LeftFoot, lf, 0);
                SetIKPos(AvatarIKGoal.RightFoot, rf, 0);
        ...
        {
            lh1 = lh1;
                SetIKPos(AvatarIKGoal.LeftHand, lh1, 0);
                SetIKPos(AvatarIKGoal.RightHand, rh1, 0);
                SetIKPos(AvatarIKGoal.LeftFoot, lf1, 0);
                SetIKPos(AvatarIKGoal.RightFoot, rf1, 0);
        }*/
